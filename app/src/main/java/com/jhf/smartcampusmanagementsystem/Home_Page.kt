...
class Home_Page : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val name = sharedPref.getString("name", "Student Name") ?: "Student Name"
        ...
        binding.bottomNavigationView.setOnItemSelectedListener { item ->
            when(item.itemId){
                R.id.home -> { replaceFragment(Student_Dashboard()); true }
                R.id.class_schedule -> { replaceFragment(Student_Cls_TimeTable()); true }
                R.id.result -> { replaceFragment(Student_result()); true }
                R.id.profile -> { replaceFragment(Student_Profile_Fragment()); true }
                else -> false
            }
        }
